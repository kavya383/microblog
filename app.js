...
function displayFeed() {
  ...
  if (visiblePosts.length === 0) {
    feedDiv.innerHTML =
      "<p>No posts to display. Follow users or create a post!</p>";
  } else {
...
function updateUserStats() {
  ...
  const followerCount = users.filter((user) =>
...
function createPostDiv(post) {
  ...
  postDiv.innerHTML = `
        ...
        <button class="like-button" onclick="likePost(${post.id})">
            ${currentUser.likedPosts.includes(post.id) ? "❤️" : "🤍"} (${
    post.likes
  })
        </button>
        ...
        <div class="comments">
            <h5>Comments</h5>
            <div id="comments-${post.id}">${createCommentsHtml(
    post.comments
  )}</div>
...
function followUser(userId, buttonElement) {
  ...
  buttonElement.innerText = currentUser.following.includes(userId)
    ? "Unfollow"
